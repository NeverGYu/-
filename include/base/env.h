...
public:
    ...
    /**
     *  @brief 初始化-->包括记录程序名称与路径、解析命令行选项与参数
     *  @details 命令行选项全部以-开头，后面跟可选参数，选项与参数构成key-value结构，被存储到程序的自定义环境变量中，如果只有key没有value，那么value为空字符串
     *  @param argc main函数传入
     *  @param argv main函数传入
     *  @return 是否成功
     */
...
private:
    ...
    std::vector<std::pair<std::string, std::string>> m_helps;   // 每一个环境变量对应的描述
    std::string m_program;                                       // 程序名称
    std::string m_exe;                                          // 程序完整路径名称
